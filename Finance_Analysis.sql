...
select count(accountid)as no_of_accounts from accounts;

'3. What is the total balance of all checking accounts?'
select sum(balance) as totalbalance,AccountType from accounts
where AccountType='checking';

'4. What is the total balance of all accounts associated with customers who live in Los Angeles?'
select sum(a.balance) as total_balance,b.city,b.CustomerID from accounts as a
join customers as b
...
select Avg(a.Balance) as Avg_balance, a.BranchID,b.branchname from Accounts as a
join branches as b
...
select CustomerID,balance,accounttype from accounts
where accounttype='checking'
order by balance desc
limit 1;

'7. Which customer has made the most transactions in the Transactions table?'
select a.customerid,sum(b.transactionid) as mosttransactions from accounts as a
inner join transactions as b
on a.AccountID=b.AccountID
inner join customers as c
on c.CustomerID=a.CustomerID
group by c.CustomerID,b.transactionid
order by mosttransactions;

'8.Which branch has the highest total balance across all of its accounts?'
select sum(a.Balance) as totalbalance, a.BranchID,b.branchname from Accounts as a
join branches as b
on a.branchid=b.branchid
group by branchid
...
select sum(a.balance) as totalbalance,a.customerid from accounts as a
...
select count(b.transactionid) as totaltransactions,a.BranchID from accounts as a
